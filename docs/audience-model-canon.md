...
ICP tension никогда не закрывается через совет или мета-категорию. Она закрывается **косвенно** — через конкретику продукта.

**Правило:** «Я сделал продукт для <buyer> и вот почему он БУДЕТ платить N $$$ в месяц.» Это и есть ответ на вопрос «что строить». Не «посмотри на API» — «вот продукт для конкретного человека, вот его боль, вот экономика.»

### Антипример

Tension: «Я умею кодить, но не знаю, что строить.»

❌ «Вот 5 API, на которых можно построить бизнес.» — список, не продукт. Совет, а не демонстрация.

❌ «Научись видеть buyer за возможностями.» — объяснение метода, а не показ продукта. Зритель остаётся в роли ученика.

❌ «Product thinking — это когда ты смотришь на API и думаешь, кому это нужно.» — определение, не evidence.

❌ «Я не придумал продукт. Я его нашёл.» — мета-процесс («как я искал»), а не конкретный результат («для кого я сделал»).

### Пример

✅ «Я сделал продукт для performance marketer'а. Он платит $149/мес.» — конкретный buyer, конкретный продукт, конкретная цифра. Ответ на «что строить» дан через демонстрацию, а не через совет.

✅ «Ему нужно 50 креативов в неделю. Студия делает 5 за $500. Я делаю за $0.75.» — боль, альтернатива, экономика. Зритель сам выводит метод из evidence.

**Принцип: show the product, don't explain the method.** Метод (product thinking) зритель выводит сам из последовательности конкретных примеров. Весь discovery работает по этой модели: каждый ролик — конкретный buyer → конкретный продукт → конкретные цифры → конкретная economics.
